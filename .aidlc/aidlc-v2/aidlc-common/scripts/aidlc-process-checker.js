...
function readSkillFrontmatterFlag(stageName, flagName) {
  const skillPath = path.join(INSTALL_ROOT, 'skills', `aidlc-${stageName}`, 'SKILL.md')
...
function nextStepForSkill(currentStep, stageName) {
  ...
  if (currentStep === 'validation') {
    const artefactVerification = readSkillFrontmatterFlag(stageName, 'artefact-verification')
...
const validationResultFile = path.join(stageOutputDir, `${stageName}-validation-result.md`)
...
const validationSpecFile = path.join('.kiro', 'skills', skillFolder, 'validation-spec.md')
    ...
    if (line.includes(`| ${stateKey} `)) {
      ...
      if (cols.length >= 4) {
        return {
          ...
          artifacts: cols[4] && cols[4] !== '—' ? cols[4].split(',').map((a) => a.trim()) : [],
        }
      }
    }
  }
...
function parseValidationResult() {
  ...
  const blockMatch = content.match(/---PROCESS-CHECK-DATA---([\s\S]*?)---END-PROCESS-CHECK-DATA---/)
...
function getToolScripts() {
  ...
  return files.filter(
    (f) => f !== '.gitkeep' && (f.endsWith('.sh') || f.endsWith('.py') || f.endsWith('.js')),
  )
}
...
function checkClarification() {
  ...
  if (state.step === 'planning') {
    addDetail(`Clarification complete — builder advanced to planning:${state.status}`)
    return
  }
  ...
  const validStatuses = ['pending', 'awaiting-human', 'answered', 'follow-up', 'complete']
...
function checkPlanning() {
  ...
  const validStatuses = ['pending', 'awaiting-human', 'revision-requested', 'approved']
  if (!validStatuses.includes(state.status)) {
...
function checkExecution() {
  ...
  if (state.step !== 'execution' || state.status !== 'complete') {
    addFailure(`Expected 'execution:complete', found '${state.step}:${state.status}'`)
    return
  }
...
function checkValidation() {
  ...
  if (result.status && result.status !== state.status) {
    addFailure(`State says '${state.status}' but result says '${result.status}'`)
  }
  ...
  if (expectedTools.length > 0) {
    ...
    for (const tool of expectedTools) {
      const found = result.toolsInvoked.some((t) => t.includes(tool) || path.basename(t) === tool)
      if (!found) {
  ...
  if (ruleCount > 0) {
    ...
    if (missing.length > 0) {
      addFailure(`Validation result missing rule numbers: ${missing.join(', ')}`)
    } else {
...
function checkStageComplete() {
  ...
  if (state.artifacts && state.artifacts.length > 0) {
    for (const artifact of state.artifacts) {
      ...
      if (!fileNonEmpty(inStageDir) && !fileNonEmpty(inIntentDir) && !fileNonEmpty(asRelative)) {
  ...
  if (fileExists(auditFile)) {
    ...
    const stageAuditLines = auditContent.split('\n').filter((l) => l.includes(`| ${stateKey} `))
...
// Update the checkpoint file
{
  ...
  const updatedCheckpoint = {
    workflow: freshWorkflow.length > 0 ? freshWorkflow : existingCheckpoint.workflow || [],
    intentDir,
    rulesDir,
    current: {
      skill: stageName,
    ...
    next: passed ? { step: nextStepForSkill(expectedStep, stageName) } : { step: expectedStep },
    error: passed
      ? null
      : {
          type: 'check-failed',
