...
function Button({
  ...
  const hasLoadingSlot = isLoading !== undefined
  ...
  return (
    ...
    >
      {hasLoadingSlot ? (
        <span aria-hidden="true" data-slot="loading-spacer" className="size-4 shrink-0" />
      ) : null}
      {children}
      {hasLoadingSlot ? (
        <LoaderCircleIcon
          aria-hidden="true"
          className={cn('size-4', isLoading ? 'animate-spin' : 'opacity-0')}
        />
      ) : null}
    </ButtonPrimitive>
  )
